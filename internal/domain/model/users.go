...
var UserWhere = struct {
	...
	Status:     whereHelperint8{field: "`users`.`status`"},
...
func (r *userR) GetUserType() *UserType {
	if r == nil {
		return nil
	}
	return r.UserType
...
// UserType pointed to by the foreign key.
func (o *User) UserType(mods ...qm.QueryMod) userTypeQuery {
	queryMods := []qm.QueryMod{
		qm.Where("`id` = ?", o.UserTypeID),
	}
...
// LoadUserType allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (userL) LoadUserType(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	if singular {
		...
		args = append(args, object.UserTypeID)
	...
	Outer:
		for _, obj := range slice {
			...
			args = append(args, obj.UserTypeID)
	...
	query := NewQuery(
		qm.From(`user_types`),
		qm.WhereIn(`user_types.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for user_types")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_types")
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserTypeID == foreign.ID {
				local.R.UserType = foreign
				if foreign.R == nil {
					foreign.R = &userTypeR{}
				}
...
// SetUserType of the user to the related item.
// Sets o.R.UserType to related.
// Adds o to related.R.Users.
func (o *User) SetUserType(ctx context.Context, exec boil.ContextExecutor, insert bool, related *UserType) error {
	var err error
	if insert {
