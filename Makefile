.PHONY: help setup build build-go clean tidy start up down restart exec logs ps setEnv lint dlint test
...
build-go: clean ## Build go file
	...
	env GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -trimpath -o bin/employee cmd/employee/main.go
...
start: ## Start a local server
	docker compose exec -it app sls offline start
